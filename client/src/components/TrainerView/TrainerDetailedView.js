    ...
    return (
        <Stack direction={'column'} sx={{width: '75%', border:2, backgroundColor:'#000', paddingTop:5, borderColor:'#6EC72D', borderRadius:10, marginTop:'15vh'}}>
            ...
            <Box sx={{marginTop:10,marginLeft:'-15%'}}>
                <Typography variant="h4"  sx={{ fontWeight: 'bolder', color:'#fff'}} align='left'>{trainer?.fullName}</Typography>
                <Typography align='left'sx={{color:'#6EC72D'}} >{trainer?.qualification}</Typography>
                <Typography variant="h6" sx={{ fontWeight: 'bolder', color:'#757575', fontFamily:'initial'}} align='left'>{trainer?.service.service} Instructor</Typography>
                <Typography align='left' component="legend"></Typography>
                <Rating name="read-only" value={5} readOnly sx={{marginLeft:-14,  }} />          
            </Box>
