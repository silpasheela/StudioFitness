...
function getIconForStatus(status) {
    switch (status) {
        ...
        case 'Normal':
            return <SentimentSatisfiedAltIcon style={{ color: 'green', ...iconStyle }} />;
        case 'Overweight':
            return <SentimentDissatisfiedIcon style={{ color: 'red', ...iconStyle }} />;
        case 'Obese':
...
function BMICalculator() {
    ...
    const handleCalculateBMI = async () => {
        ...
        try {
            const response = await instance.post('user/calculate-bmi', { height, weight });
    ...
    return (
        <div style={containerStyle}>
            <Typography variant="h4" gutterBottom sx={{ color: '#000', fontWeight: 'bolder', paddingTop: '15px' }}>
                BMI Calculator
            </Typography>
            <TextField
                label="Height"
